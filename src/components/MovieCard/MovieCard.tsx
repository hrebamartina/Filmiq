        ...
        <section className={styles.movieCard}>
            ...
            <div className={styles.detailsAndActions}>
                <div className={styles.details}>
                    ...
                    <p className={styles.title}><strong>Year:</strong> {movie.release_date?.slice(0, 4)}</p>
                    <p className={styles.description}>{movie.overview || "No description available."}</p>

                    <div className={styles.tabs}>
                        <button onClick={() => setActiveTab('cast')} className={activeTab === 'cast'? styles.active : ""}>Cast</button>
                        <button onClick={() => setActiveTab('crew')} className={activeTab === 'crew' ? styles.active: ""}>Crew</button>
                        <button onClick={() => setActiveTab('genres')} className={activeTab === 'genres' ? styles.active: ""}>Genres</button>
                    </div>

                    {activeTab === "cast" && (
                        <ul>
                            {movie.credits?.cast?.slice(0, 10).map(c => (
                                <li key={c.id}>{c.name} {c.character ? `-${c.character}` : "" }</li>
                            ))}
                        </ul>
                    )}
                    ...
                    {activeTab === "crew" && (
                        <ul>
                            {movie.credits?.crew?.slice(0,10).map(c=> (
                                <li key={c.id}>{c.name} {c.job ? `-${c.job}` : ""}</li>
                            ) )}
                        </ul>
                    )}
                    {activeTab === "genres" && (
